...
const COPY = {
  en: {
    ...
    noneYet: "None yet",
    edit: "Edit",
    rateLimited: (resetInMinutes: number) => `GitHub API rate limited — showing cached data. Resets in ${resetInMinutes}m`,
    rateLow: (remaining: number, limit: number, resetInMinutes: number) => `GitHub API: ${remaining}/${limit} remaining. Resets in ${resetInMinutes}m`,
    issuesHeader: (count: number) => `Issues (${count})`,
    prsHeader: (count: number) => `Pull Requests (${count})`,
    issuesTooltipBefore: "Issues",
    issuesTooltipAfter: "— open issues on the project's GitHub repo. Click any item to open it on GitHub.",
    prsTooltipBefore: "Pull Requests",
    prsTooltipAfter: "— open PRs awaiting review or merge. Click to open on GitHub.",
    queueTooltipBefore: "Overnight Queue",
    queueTooltipAfter: "— the task queue file Head reads to pick the next ticket. Click Edit to modify batch contents and ordering.",
  },
  ko: {
    noIssues: "이슈 없음",
    noPRs: "PR 없음",
    ...
    noneYet: "아직 없음",
    edit: "편집",
    rateLimited: (resetInMinutes: number) => `GitHub API 제한에 걸렸습니다 - 캐시된 데이터를 표시합니다. ${resetInMinutes}분 후 초기화됩니다`,
    rateLow: (remaining: number, limit: number, resetInMinutes: number) => `GitHub API 남음: ${remaining}/${limit}. ${resetInMinutes}분 후 초기화`,
    issuesHeader: (count: number) => `이슈 (${count})`,
    prsHeader: (count: number) => `풀 리퀘스트 (${count})`,
    issuesTooltipBefore: "이슈",
    issuesTooltipAfter: "- 이 프로젝트 GitHub 저장소의 열린 이슈입니다. 항목을 클릭하면 GitHub에서 열립니다.",
    prsTooltipBefore: "풀 리퀘스트",
    prsTooltipAfter: "- 검토 또는 병합을 기다리는 열린 PR입니다. 클릭하면 GitHub에서 열립니다.",
    queueTooltipBefore: "야간 큐",
    queueTooltipAfter: "- Head가 다음 티켓을 고를 때 읽는 작업 큐 파일입니다. 편집을 눌러 배치 내용과 순서를 수정할 수 있습니다.",
  },
} as const;
