...
const COPY = {
  en: {
    ...
    koHelpClass: "",
    steps: [
      { title: "You assign a task in the chat", body: "Tell @head what to build. Be as specific or as vague as you like." },
      { title: "Head creates a GitHub issue", body: "Head opens an issue, adds it to the queue, and waits for your trigger." },
      { title: "Dev writes the code", body: "Dev clones a branch, implements the change, and opens a pull request." },
      { title: "Reviewers check the work", body: "RE1 and RE2 each review the PR independently. Both must approve before the PR is mergeable." },
      { title: "Head merges and continues", body: "Head merges the approved PR and assigns the next ticket from the queue. The cycle continues all night while you sleep." },
    ],
  },
  ko: {
    ...
    koHelpClass: "ko-help",
    steps: [
      { title: "채팅에서 작업을 지시합니다", body: "@head 에게 무엇을 만들지 말해 주세요. 아주 구체적으로 써도 되고, 느슨하게 지시해도 됩니다." },
      { title: "Head가 GitHub 이슈를 만듭니다", body: "Head가 이슈를 열고, 큐에 추가한 뒤, 당신의 트리거를 기다립니다." },
      { title: "Dev가 코드를 작성합니다", body: "Dev가 브랜치를 만들고, 변경 사항을 구현한 뒤, 풀 리퀘스트를 엽니다." },
      { title: "리뷰어가 작업을 검토합니다", body: "RE1과 RE2가 각각 독립적으로 PR을 리뷰합니다. 둘 다 승인해야 PR이 병합 가능 상태가 됩니다." },
      { title: "Head가 병합하고 계속 진행합니다", body: "Head가 승인된 PR을 병합하고, 큐에서 다음 티켓을 할당합니다. 당신이 자는 동안에도 이 사이클은 밤새 계속됩니다." },
    ],
  },
} as const;
...
export default function HowToWorkModal({ open, onClose }: HowToWorkModalProps) {
  ...
  return (
    ...
    >
      <div
        className={`relative mx-4 max-w-xl w-full max-h-[90vh] overflow-auto rounded-lg border border-white/10 bg-neutral-950 p-6 shadow-2xl ${t.koHelpClass}`}
